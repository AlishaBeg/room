...
class Client:
    ...
    __SOCKET = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # collection (IPv4 and TCP)
        ...
        try:
            while True:
                ...
                if request.strip().upper() == "EXIT":
                    Client.__SOCKET.sendall("Please quit me")
                    break
    ...
    def active():
        try:
            ...
            print(f"~ server [http://{Client.__HOST}:{Client.__PORT}] is (connected)")
